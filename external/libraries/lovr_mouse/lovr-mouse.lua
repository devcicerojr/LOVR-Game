...
function mouse.setVisible(visible)
  C.glfwSetInputMode(window, C.GLFW_CURSOR, visible and C.GLFW_CURSOR_NORMAL or C.GLFW_CURSOR_HIDDEN)
end

function mouse.newCursor(source, hotx, hoty)
  if type(source) == 'string' or tostring(source):find('Blob') then
    source = lovr.data.newImage(source)
  else
    assert(tostring(source):find('Image'), 'Bad argument #1 to newCursor (Image expected)')
  end
