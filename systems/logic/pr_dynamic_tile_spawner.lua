...
return {
  ...
  update_fn = function(ecs, id, c, dt) --update function
    ...
    if player_pos_z - pos_z >= 200 then
      pr_event_bus:emit('dynamic_tile_despawned', ecs, id)
      
      local spawn_pos = lovr.math.newVec3(pos_x, pos_y, pos_z + 400)
      local spawned_tile = nil
      if spawn_pos.x == -10 then
        spawned_tile = (require'../entities/tiles/pr_tiled_ground')(ecs, spawn_pos, 20, "assets/left_side_asphalt.png")
      elseif spawn_pos.x == 10 then
        spawned_tile = (require'../entities/tiles/pr_tiled_ground')(ecs, spawn_pos, 20, "assets/right_side_asphalt.png")
      else
        spawned_tile = (require'../entities/tiles/pr_tiled_ground')(ecs, spawn_pos)
      end
      if pos_z > latest_z_val then
        latest_z_val = pos_z
        pr_event_bus:emit('terrain_tile_spawned', ecs, spawn_pos)
      end
    end
