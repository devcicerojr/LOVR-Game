...
return {
  ...
  update_fn = function(ecs, id, c, dt) --update function
    ...
    if desired_dir:length() == 0 then
      desired_dir = vec3(0, 0, 1) -- default forward direction
    else
    ...
    if desired_speed > 0 then
      ...
      acc_dec.current_speed = lovr.math.newVec3(vec3(desired_dir) * current_speed_len + accel_vec)
      -- Clamp to max speed
      if acc_dec.current_speed:length() > desired_speed then
        acc_dec.current_speed = lovr.math.vec3(acc_dec.current_speed):normalize() * desired_speed
      end
    else
    ...
    if collider:isKinematic() then
      ...
      if collided_c ~= nil then
        ...
        local left_sensor_pos = lovr.math.vec3(aabb_sensor_pos) + lovr.math.vec3(col_width/2, col_height / 2, depth_offset_sig * col_depth/2):rotate(orientation)
        ...
        local right_sensor_pos = lovr.math.vec3(aabb_sensor_pos) + lovr.math.vec3(-col_width/2, col_height/2, depth_offset_sig * col_depth/2):rotate(orientation)
      ...
      if acc_dec.current_speed:length() > 0 then
        ...
        translate_val = direction * dt
        collider:setPosition(lovr.math.vec3(collider:getPosition()) + translate_val)
      end
    end
  end
}